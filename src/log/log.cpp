...
bool Log::init(const char *file_name,int close_log,int log_buf_size,int split_lines,int max_queue_size){
    ...
    if(p==NULL){
        ...
        snprintf(log_full_name, 255, "%d_%02d_%02d_%s", m_tm.tm_year + 1900, m_tm.tm_mon + 1, m_tm.tm_mday, file_name);
    }else{
        ...
        // 生成完整的日志文件名，包含路径和文件名
        snprintf(log_full_name, 255, "%s%d_%02d_%02d_%s", dir_name,m_tm.tm_year + 1900, m_tm.tm_mon + 1, m_tm.tm_mday, log_name);
    }
...
void Log::write_log(int level,const char *format,...){
    ...
    int n = snprintf(m_buf, 48, "%d-%02d-%02d %02d:%02d:%02d.%06ld %s",
                     m_tm.tm_year, m_tm.tm_mon + 1, m_tm.tm_mday, 
